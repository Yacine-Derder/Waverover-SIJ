...
int main(int argc, char **argv) {
  ...
  node->declare_parameter<int>("scan_points", setting.scan_points);
  ...
  node->get_parameter("scan_points", setting.scan_points);
...
void  ToLaserscanMessagePublish(ldlidar::Points2D& src,  double lidar_spin_freq, LaserScanSetting& setting,
  ...
  if (setting.scan_points > 0) {
    beam_size = setting.scan_points;
  }
