...
float CLaserOdometry2D::scanAngle(unsigned int beam_index, unsigned int beam_count) const
{
  if (beam_count <= 1)
    return scan_angle_min;

  return scan_angle_min + float(beam_index) * fovh / float(beam_count - 1);
}

float CLaserOdometry2D::scanIndex(float angle, unsigned int beam_count) const
{
  if (beam_count <= 1 || fovh <= 0.f)
    return 0.f;

  const float two_pi = 2.f * static_cast<float>(M_PI);
  if (fovh >= two_pi - 1e-3f)
  {
    while (angle < scan_angle_min)
      angle += two_pi;
    while (angle > scan_angle_min + fovh)
      angle -= two_pi;
  }

  return (angle - scan_angle_min) * float(beam_count - 1) / fovh;
}
...
void CLaserOdometry2D::createImagePyramid()
{
  ...
  for (unsigned int i = 0; i<pyr_levels; i++)
  {
    ...
    for (unsigned int u = 0; u < cols_i; u++)
    {
      if (range[i](u) > 0.f)
      {
        const float tita = scanAngle(u, cols_i);
...
void CLaserOdometry2D::performWarping()
{
  ...
  for (unsigned int j = 0; j<cols_i; j++)
  {
    if (range[image_level](j) > 0.f)
    {
      ...
      //Calculate warping
      const float uwarp = scanIndex(tita_w, cols_i);
